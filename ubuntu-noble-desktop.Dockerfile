...
RUN apt update && apt install -y sudo dbus dbus-broker dnsutils iputils-ping jq wget build-essential python3 python3-pip jq wireguard wireguard-tools resolvconf jq libfuse2 libxi6 libxrender1 libxtst6 mesa-utils libfontconfig libgtk-3-bin
RUN echo "#1000 ALL=(ALL:ALL) NOPASSWD:ALL" >> /etc/sudoers

RUN sh -c "$(curl -fsSL https://thmr.at/setup/netbird)"
